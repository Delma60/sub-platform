...
export function ProfileForm({ initialName, email }: { initialName: string; email: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok || !json.success) {
        setMessage({ type: "error", text: json.error ?? "Couldn't update profile." });
        return;
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3 rounded-3xl border border-[#E4DCC8] bg-white p-6">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        />
        <p className="text-xs text-[#6B6558]">Contact support to change your email.</p>
      </div>

      {message && (
        <p
          className={`rounded-md px-3 py-2 text-sm ${
            message.type === "success" ? "bg-[#24402F]/10 text-[#24402F]" : "bg-red-50 text-red-700"
          }`}
        >
          {message.text}
        </p>
